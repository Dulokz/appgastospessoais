          ...
          {cards.map((card) => (
            <div key={card.id} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1"><h2 className="font-bold text-white">{card.name}</h2><p className="text-xs text-muted-foreground">Emitido por {card.institution}</p></div>
              ...
              <div className="mt-5 pt-3 border-t border-white/10"><p className="text-[11px] text-muted-foreground">Dívida registrada</p><p className="text-2xl font-black text-rose-400">{formatCurrencyBRL(Math.max(0, -card.balance))}</p></div>
            </div>
          ))}
        </div>
      )}
