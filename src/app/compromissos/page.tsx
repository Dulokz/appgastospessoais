...
export default async function CompromissosPage() {
  ...
  const [accounts, categories, commitments] = await Promise.all([
    ...
    db.scheduledCommitment.findMany({ where: { userId, status: "PENDING" }, include: { account: { include: { financialInstitution: true } }, category: { include: { parent: true } } }, orderBy: { dueDate: "asc" }, take: 80 }),
  ]);
